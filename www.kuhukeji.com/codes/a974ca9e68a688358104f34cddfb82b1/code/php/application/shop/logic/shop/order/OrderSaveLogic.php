    ...
    public function getPayOrderInfo($openId, $device, $payType = '')
    {
        ...
        if ($is_pay == 1) {
            $param = [
                ...
                'subject' => "商品购买",
                'openid' => $openId,
                'notify_url' => "https://demo.woyaou.com" . config('callbackUrl.shop_payment_notify_callback').'/appid/'.$this->appId . '/device/' . $device . '/pay_type/' . $payType . '/order_id/' . $this->orderModel->order_id,
